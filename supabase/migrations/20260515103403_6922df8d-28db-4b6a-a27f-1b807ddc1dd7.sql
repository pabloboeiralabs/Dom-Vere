CREATE OR REPLACE FUNCTION public.get_professional_history(p_user_id uuid, p_professional_id uuid)
 RETURNS TABLE(record_type text, record_date timestamp with time zone, customer_name text, service_name text, amount numeric, notes text)
 LANGUAGE sql
 STABLE
 SECURITY DEFINER
...
AS $function$
  SELECT * FROM (
    ...
    SELECT 'cut'::text, cu.created_at, COALESCE(c.name, 'Anônimo'), '—', 0::numeric, cu.notes
    FROM cuts cu LEFT JOIN customers c ON c.id = cu.customer_id
    WHERE cu.user_id = p_user_id AND cu.professional_id = p_professional_id

    UNION ALL

    SELECT 'plan_usage'::text, pur.created_at, COALESCE(cust.name, 'Anônimo'),
      string_agg(pus.service_name, ', '),
      CASE WHEN COALESCE(cp.usage_limit, 0) > 0
           THEN (COALESCE(cp.total_price, 0) / cp.usage_limit)::numeric
           ELSE 0::numeric END,
    ...
    GROUP BY pur.id, pur.created_at, cust.name, pl.name, cp.total_price, cp.usage_limit

    UNION ALL

    SELECT 'transaction'::text, t.created_at, COALESCE(c.name, 'Anônimo'), '—',
